...
export function generateContractsDocx(dataList: ExtractedData[]): Document[] {
  ...
  const documents = dataList.map((data) => {
    return new Document({
      sections: [{
        properties: {},
        children: [
          ...
          new Paragraph({
            ...
            children: [
              ...
              new TextRun({
                text: `${data.nome ? data.nome.toUpperCase() : ""}`,
                bold: true,
              }),
              new TextRun({
                text: `, ${data.nacionalidade ? data.nacionalidade.toLowerCase() : ""}, ${data.estadoCivil ? data.estadoCivil.toLowerCase() : ""}, ${data.profissao ? data.profissao.toLowerCase() : ""}, regularmente inscrito no CPF sob o nº ${data.cpf || "_________"}, residente e domiciliado(a) na ${data.endereco || "_________"}, CEP: ${data.cep || "_________"}, na cidade de ${data.cidade || "_________"}/${data.estado || "_________"}, de ora em diante denominado apenas ASSOCIADO.`,
              }),
            ],
            spacing: { after: 200 },
          }),
          ...
          new Paragraph({
            alignment: AlignmentType.CENTER,
            children: [
              new TextRun({ text: `${data.nome ? data.nome.toUpperCase() : ""}`, bold: true })
            ],
            spacing: { after: 600 },
          }),
          new Paragraph({
