...
export function generateContractPdf(data: ExtractedData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.font("Helvetica-Bold").text(`${data.nome.toUpperCase()}`, { continued: true });
      doc.font("Helvetica").text(`, ${data.nacionalidade.toLowerCase()}, ${data.estadoCivil.toLowerCase()}, ${data.profissao.toLowerCase()}, regularmente inscrito no CPF sob o nº ${data.cpf}, residente e domiciliado(a) na ${data.endereco}, CEP: ${data.cep}, na cidade de ${data.cidade}/${data.estado}, de ora em diante denominado apenas ASSOCIADO.`, { align: "justify" });
      doc.moveDown(0.5);
      ...
      doc.font("Helvetica-Bold").text(`${data.nome.toUpperCase()}`, { align: "center" });
      doc.moveDown(3);
